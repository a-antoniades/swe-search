...
def test_find_by_line_numbers():
    ...
    file = workspace.file_repo.get_file("sklearn/compose/tests/test_column_transformer.py")
    ...
    assert block_before
    assert block_before.path_string() == "test_column_transformer_sparse_stacking.assert_array_equal_X_tran_3"
    assert block_after
    assert block_after.identifier == "test_column_transformer_mixed_cols_sparse"

    parent_structure_block = block_before.find_type_group_in_parents(CodeBlockTypeGroup.STRUCTURE)
    assert parent_structure_block
    assert parent_structure_block.path_string() == "test_column_transformer_sparse_stacking"
