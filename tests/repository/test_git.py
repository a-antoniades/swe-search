...
def test_diff_with_ignore_paths(temp_git_repo, git_repository):
    ...
    git_repository.save_file('file2.txt', 'Initial content for file2\nAdded content to file2')
    git_repository.save_file('ignore_me.txt', 'This file should be ignored\nAdded content to ignore_me')
...
def test_snapshot_and_restore(temp_git_repo, git_repository):
    ...
    assert (temp_git_repo / 'file1.txt').read_text() == 'Initial content for file1\nNew content'
