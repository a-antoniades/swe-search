from moatless.benchmark.swebench import setup_swebench_repo, create_workspace, create_index
...
def test_all_instance_test_fils():
    ...
    for instance_id, instance in instances.items():
        ...
        for expected_file in instance["expected_spans"].keys():
            assert not is_test(expected_file), f"{expected_file} for {instance_id} is a test file"

    for missed_test_file in missed_test_files:
        print(missed_test_file)

    assert len(missed_test_files) < 25, f"Expected less than 25 missed test files. Got {len(missed_test_files)} missed matches."
...
def test_find_test_files():
    ...
    files = workspace.code_index.find_test_files("sympy/printing/ccode.py", max_results=3)
    assert len(files) == 3
    assert files == {'sympy/utilities/tests/test_codegen.py', 'sympy/printing/tests/test_fcode.py', 'sympy/printing/tests/test_ccode.py'}
...
def test_find_test_files_with_filename_match_but_low_semantic_rank():
    ...
    files = workspace.code_index.find_test_files("sympy/polys/domains/polynomialring.py", max_results=3)
    assert len(files) == 3
...
def test_find_test_files_by_span():
    ...
    files = workspace.code_index.find_test_files("django/db/models/fields/related.py", span_id="ForeignKey.formfield", max_results=3)
    assert len(files) == 3
...
def test_find_test_function():
    ...
    files = workspace.code_index.find_test_files("django/dispatch/dispatcher.py", max_results=3, max_spans=1)
    for file in files:
...
def test_find_test_function():
    ...
    query = """set_ticks"""
    files = workspace.code_index.find_test_files("django/utils/text.py", query=query, max_results=3, max_spans=1)
    for file in files:
        print(file)
        assert "test" in file.span_ids[0].lower()
...
def test_wildcard_file_patterh():
    ...
    results = code_index.search(query, file_pattern=file_pattern, max_results=250, max_tokens=8000)
