...
from moatless.schema import VerificationIssueType, TestStatus
...
def test_syntax_error():
    ...
    testbed = TestbedVerifier(testbed=None, repository=file_repo)
    ...
    assert [(relevant_file.file_path, relevant_file.span_id) for relevant_file in issue.relevant_files[:4]] == [
...
def test_django_errors():
    ...
    errors = [issue for issue in results if issue.status == TestStatus.ERROR and issue.message]
    assert 3 == len(errors)

    for error in errors:
        assert error.relevant_files[0].file_path == "django/forms/widgets.py", f"Expected first relevant file to be 'widgets.py', but got '{error.relevant_files[0].file_path}'"
    ...
    failures = [issue for issue in results if issue.status == TestStatus.FAILED and issue.message]
    assert 4 == len(failures)
    assert [(failure.file_path, failure.span_id) for failure in failures] == [
        ('tests/forms_tests/tests/test_media.py', 'FormsMediaTestCase.test_media_inheritance_from_property'),
    ...
    for failure in failures:
        if failure.relevant_files:
            assert failure.relevant_files[0].file_path == failure.file_path, f"Expected first relevant file to be '{failure.file_path}', but got '{error.relevant_files[0].file_path}'"
            assert failure.relevant_files[0].span_id == failure.span_id, f"Expected first relevant file to be '{failure.span_id}', but got '{error.relevant_files[0].span_id}'"
...
def test_pylint_failures():
    ...
    issues = [issue for issue in results if issue.status in [TestStatus.FAILED, TestStatus.ERROR] and issue.message]
    assert len(issues) == 1
