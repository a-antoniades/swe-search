...
class TestEditCode:
    ...
    def test_required_fields(self, edit_code: EditCode):
        assert edit_code.required_fields() == {"instructions", "file_path", "start_line", "end_line"}
...
def test_expect_failed_edit():
    ...
    def create_state():
        ...
        workspace = create_workspace(instance)
        workspace.file_context.add_spans_to_context("src/_pytest/mark/evaluate.py",
                                                    ["imports", "cached_eval", "MarkEvaluator"])

        edit_code = EditCode(id=0, _workspace=workspace,
                             verify=False,
                             instructions="Test",
    ...
    edit_code = create_state()
    outcome = edit_code.execute(Content(content="To remove the `cached_eval` function as requested, I'll simply return an empty <replace> tag:\n\n<replace>\n"))
    assert outcome.trigger == "finish"
    assert "-def cached_eval" in outcome.output["diff"]
...
def test_incomplete_replace():
    ...
    workspace = create_workspace(instance)
    workspace.file_context.add_spans_to_context("sympy/sets/sets.py",
                                                ["imports", "imageset"])
...
def test_wrong_indentation():
    instance = get_moatless_instance("django__django-14238", split="lite")
    workspace = create_workspace(instance)
    workspace.file_context.add_spans_to_context("django/db/models/fields/__init__.py", ["AutoFieldMeta", "AutoFieldMeta._subclasses"])

    edit_code = EditCode(id=0, model="", _workspace=workspace,
                         instructions="Test",
                         change_type=ChangeType.modification,
...
"""
    ...
    outcome = edit_code.execute(Content(content=replace_content))
    assert outcome.trigger == "retry"
    assert outcome.retry_message.startswith("The code in the <replace> tag has an indentation of 0 spaces while the code in the <search> tag has 4 spaces.")
