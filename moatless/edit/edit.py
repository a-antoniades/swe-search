...
class EditCode(AgenticState):
    ...
    def _execute_action(self, content: Content) -> StateOutcome:
        ...
        if len(msg_split) == 1:
            if not self._add_prepared_response:
                logger.warning(
                    f"{self.trace_name} No <replace> tag found in response without prepped tag: {msg_split[0]}"
                )
        ...
        if not replacement_code.strip():
            return self.retry(
                "The code in the replace tag is empty. If you can't do any changes and want to reject the instructions, use the reject function."
            )

        file = self.file_repo.get_file(self.file_path)
        if not file:
            logger.info(f"File not found: {self.file_path}. Saving new file.")
            updated_content = replacement_code
            diff = do_diff(self.file_path, "", updated_content)
            existing_span_ids = set()
        else:
            context_file = self.file_context.get_file(self.file_path)
            if context_file.content:
            ...
            diff = do_diff(self.file_path, file.content, updated_content)
            existing_span_ids = file.module.get_all_span_ids()

        if not diff:
            logger.info(f"{self.trace_name}: No changes in {self.file_path}.")
            return self.retry(
        ...
        invalid_update_str = self.verify_change(updated_content, replacement_code, file=file)
        if invalid_update_str:
            logger.warning(
                f"{self.trace_name} Invalid update in {self.file_path}: {invalid_update_str}.\nDiff:\n{diff}"
            )
            return self.retry(invalid_update_str)

        file = self.file_repo.save_file(
            file_path=self.file_path, updated_content=updated_content
        )
        ...
        updated_file_context = [FileWithSpans(file_path=self.file_path, span_ids=list(new_span_ids))]

        return StateOutcome.transition(
            "finish",
            output={
                "diff": diff,
                "updated_file_context": updated_file_context
            },
        )

    def retry(self, message: str) -> StateOutcome:
        if self._retry > 2:
            logger.warning(f"{self.trace_name} Failed after {self._retry} retries. Will reject change.")
    ...
    def verify_change(
        self, updated_content: str, replacement_code: str, file: CodeFile | None,
    ) -> str | None:
        ...
        try:
            ...
            logger.warning(
                f"{self.trace_name} Failed to parse updated content in {self.file_path}: {e}. Content:\n{updated_content}"
            )
            return f"Syntax is invalid in the updated code. "
        ...
        if self.change_type == ChangeType.modification:
            existing_block = self.get_block_to_replace()
            if existing_block:
                new_block = updated_module.find_first_by_start_line(self.start_line)

                logger.info(f"Compare existing block {existing_block.display_name} with new block {new_block.display_name}")

                if existing_block.indentation != new_block.indentation:
                    logger.warning(
                        f"{self.trace_name} Indentation change: {existing_block.display_name} {existing_block.indentation} -> {new_block.display_name} {new_block.indentation}"
                    )
                    return (
                        ...
                        f"Please correct this. If you shouldn't do any changes to {existing_block.identifier}, reject the request and explain why."
                    )
                ...
                if existing_block.type != new_block.type and not (
                    ...
                    logger.warning(
                        f"{self.trace_name} Full block change: {existing_block.type.value} -> {new_block.type.value}"
                    )
                    return (
    ...
    def get_block_to_replace(self):
        """
        Return the exact block to replace if one exists
        """
        file = self.file_repo.get_file(self.file_path)
        if not file:
            return None

        code_block = file.module.find_first_by_start_line(self.start_line)
        if code_block and code_block.start_line == self.start_line and code_block.end_line == self.end_line and code_block.type.group == CodeBlockTypeGroup.STRUCTURE:
            return code_block
    ...
    def create_search_block(self):
        ...
        if not code_to_replace and self.change_type != ChangeType.addition:
            logger.warning(f"No code found to replace in {self.file_path} from line {self.start_line} to {self.end_line}.")
        return f"<search>\n{code_to_replace}\n</search>\n"
    ...
    def create_message(self) -> str:
        ...
        if file:
            ...
            content += self.create_search_block()
            content += f"\nCode found on line numbers {self.start_line} to {self.end_line} in {self.file_path}:\n"
        else:
            content += "\n<search>\n# No content...\n</search>\n"
    ...
    def messages(self) -> list[Message]:
        messages = [UserMessage(content=self.create_message())]
