...
class ClarifyCodeChange(AgenticState):
    ...
    span_id: Optional[str] = Field(
        default=None, description="The ID of the span to be updated."
    )
    ...
    def init(self):
        ...
        if self.span_id:
            ...
            if not self._span:
                raise ValueError(
                    f"Span with ID {self.span_id} not found in {self.file_path}"
                )
        ...
        if self.span.initiating_block.type == CodeBlockType.CLASS:
            for child in self.span.initiating_block.children:
                if (
                    ...
                    if self.file_context.has_span(
                        file_path=self.file_path, span_id=child.belongs_to_span.span_id
                    ):
    ...
    def _verify_line_numbers(
        ...
        if (
            ...
            return (
                f"The provided line numbers {line_numbers.start_line} - {line_numbers.end_line} covers the whole code span. "
        ...
        if span_block.type.group == CodeBlockTypeGroup.STRUCTURE:
            ...
            if (
                ...
                logger.info(
                    f"{self.id}:{self.name}:  {clarify_msg}. Ask for clarification."
                )
    ...
    def get_line_span(
        ...
        if self.span_id:
            parent_block = self.file.module.find_span_by_id(
                self.span_id
            ).initiating_block
        else:
