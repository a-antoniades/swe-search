...
class CodeIndex:
    ...
    def search(
        ...
        def add_to_results(hits: List[SearchCodeHit]):
            nonlocal sum_results
            nonlocal sum_tokens

            if results:
                try:
                    base_rank = max(
                        [max([span.rank for span in file.spans]) for file in results.values() if file.spans])
                except ValueError:
                    base_rank = 0
            else:
                base_rank = 0
            ...
            for hit in hits:
                if sum_results >= max_results or sum_tokens >= max_tokens:
                    return

                if hit.file_path not in results:
                    results[hit.file_path] = SearchCodeHit(file_path=hit.file_path)

                for span in hit.spans:
                    if len(results[hit.file_path].spans) >= max_results_per_file:
                        break

                    if span.span_id not in results[hit.file_path].span_ids:
                        results[hit.file_path].add_span(span.span_id, base_rank + span.rank, span.tokens)
                        sum_results += 1
                        sum_tokens += span.tokens

        if class_names or function_names:
            add_to_results(self.find_by_name(
                class_names=class_names,
                function_names=function_names,
                file_pattern=file_pattern,
            ).hits)

            if len(results) == 0 and class_names and function_names:
                add_to_results(
                    self.find_by_name(
                        class_names=class_names,
                        file_pattern=file_pattern,
        ...
        if (query or code_snippet) and sum_tokens < max_tokens and sum_results < max_results:
            add_to_results(self.semantic_search(
                query=query,
                code_snippet=code_snippet,
                class_names=class_names,
                function_names=function_names,
                file_pattern=file_pattern,
                max_results=max_results - sum_results,
                max_tokens=max_tokens - sum_tokens,
            ).hits)
            ...
            if code_snippet:
                if not query:
                    ...
                    query += "\n" + code_snippet
                if sum_tokens < max_tokens and sum_results < max_results:
                    add_to_results(self.semantic_search(
                        query=query,
                        class_names=class_names,
                        function_names=function_names,
        ...
        logger.info(f"search() Found {sum_results} results with {sum_tokens} tokens.")

        return SearchCodeResponse(message=f"Found {len(results)} results.", hits=list(results.values()))
    ...
    def find_class(self, class_name: str, file_pattern: Optional[str] = None):
        return self.find_by_name(class_names=[class_name], file_pattern=file_pattern, strict=True)

    def find_function(self, function_name: str, class_name: Optional[str] = None, file_pattern: Optional[str] = None):
        class_names = [class_name] if class_name else None
        return self.find_by_name(function_names=[function_name], class_names=class_names, file_pattern=file_pattern, strict=True)
    ...
    def find_by_name(
        ...
        if not paths and not strict:
            if function_names:
                for function_name in function_names:
                    paths.extend(self._blocks_by_class_name.get(function_name, []))

            if class_names:
                for class_name in class_names:
                    paths.extend(self._blocks_by_function_name.get(class_name, []))
