...
from moatless.transition_rules import TransitionRule, TransitionRules, TreeSearchSettings
...
class AgenticLoop:
    def __init__(
        ...
        debate: bool = False,
        reward_threshold: Optional[float] = 100,
        ...
        **kwargs,
        ...
        if transition_rules.loop_settings:
            self._max_cost = transition_rules.loop_settings.max_cost
            self._max_transitions = transition_rules.loop_settings.max_transitions
        else:
            # Create settings for backward compatibility
            self._max_cost = max_cost
            self._max_transitions = max_transitions

        # MCTS
        if transition_rules.tree_search_settings:
            self._tree_search_settings = transition_rules.tree_search_settings
        else:
            # Create settings for backward compatibility
            self._tree_search_settings = TreeSearchSettings(
                ...
                debate=debate,
                value_function_model=Settings.cheap_model,
    ...
    @classmethod
    def from_trajectory_file(cls, trajectory_path: str, transition_rules: TransitionRules, **kwargs):
        trajectory = Trajectory.load(trajectory_path, **kwargs)

        if not transition_rules:
            transition_rules = TransitionRules(transition_rules=trajectory.transitions)
    ...
    def _execute_state_until_transition_to(self, states: List[str]) -> State | None:
        next_state = None

        while not self.is_finished():
            next_state = self._execute_state_until_transition()
            if next_state.name in states:
                return next_state
    ...
    def run_search(self, message: Optional[str] = None) -> Response:
        if self.is_running():
        ...
        mcts = MCTS(self, settings=self._tree_search_settings, **self.kwargs)
        ...
        logger.warning(f"Loop exited with unknown state {self.state.name}.")
        return Response(status="error", message="Loop exited with unknown state.")
    ...
    def _execute_state(self) -> State | None:
        ...
        if isinstance(self.state, Pending):
            ...
            try:
                ...
                logger.exception(
                    f"{self.state.trace_name} failed to execute state."
                )
                rejected_state = self._create_state(
                    Rejected, {
                        "message": "Failed to execute state.",
            ...
            if outcome.trigger == "retry":
                ...
                if isinstance(self.state, AgenticState) and self.state.retries() >= self.state.max_retries:
                    self.log_info(f"Max retries reached ({self.state.max_retries}) in {self.state.trace_name}. Reject state.")
                    trigger = "reject"
                    output = {"message": f"Rejected after max retries. Last retry messate: {outcome.retry_message}"}
                else:
                    return None
            else:
        ...
        if not transition_rule:
            raise RuntimeError(
                f"No transition rule found for {self.state.name} with trigger {trigger} and output {output}"
            )
    ...
    def revert_to_state(self, state: State | int | None = None, state_id: int | None = None):
        if isinstance(state, int):
            state_id = state
        if state_id or isinstance(state, int):
